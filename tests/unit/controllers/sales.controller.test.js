...
const { createSale, getSaleById } = require('../../../src/controllers/sales.controller');
...
describe('Sales controller', () => {
  ...
  describe('getSaleById', async () => {
    it('should return a 200 status code and a sale object if the sale exists', async () => {
      ...
      const getSaleByIdStub = sinon.stub(salesService, 'getSaleById').resolves({ id: 1, itemsSold: [] });

      await getSaleById(req, res);

      expect(res.status.calledOnceWith(200)).to.be.true;
      expect(res.status().json.calledOnceWith({ id: 1, itemsSold: [] })).to.be.true;
    ...
    it('should return a 404 status code if the sale does not exist', async () => {
      ...
      const getSaleByIdStub = sinon.stub(salesService, 'getSaleById').rejects(error);
